...
int main() {
	...
	int status = Quick_Sort_NAIVE(data, 0, data_num);

	if (status != 0) fprintf(stderr, "Sorting Error Occur\n"); //정렬이 성공적으로 이루어지지 않은 경우, 에러메시지를 출력

	printf("Sorting Type: Quick_Sort_NAIVE\n"); //정렬의 종류
	...
	*/
	...
	strcpy(file_write, "output_QS_NAIVE.bin"); //정렬된 데이터를 저장할 이진파일의 이름
...
int Quick_Sort_NAIVE(unsigned int* data, int left, int right) {
	...
	//배열의 크기가 4 이상일 때, 무조건 재귀 함수 호출
		...
		Quick_Sort_NAIVE(data, left, Pivot - 1);
		Quick_Sort_NAIVE(data, Pivot + 1, right);
	}
...
int Partition(unsigned int* data, int left, int right) {
	int Pivot = left; //Pivot으로는 가장 앞에 있는 원소를 선택
